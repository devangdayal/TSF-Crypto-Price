...
class Plotter:
    ...
    def plot_histogram_data_split(self, training_data, test_data, validation_data):
        ...
        plt.plot(training_data.Close, color='blue')
    ...
    def project_plot_predictions(self, price_predicted, test_data):
        ...
        plt.plot(price_predicted[self.stock_ticker + '_predicted'], color='green', label='Predicted [' + self.short_name + '] price')
        plt.plot(test_data.Close, color='blue', label='Actual [' + self.short_name + '] price')
        plt.xlabel('Time')
        plt.ylabel('Price [' + self.currency + ']')
        plt.legend()
